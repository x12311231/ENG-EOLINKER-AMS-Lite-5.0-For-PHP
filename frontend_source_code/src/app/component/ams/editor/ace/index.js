(function () {
    ...
    function indexController($scope,$rootScope) {
        ...
        vm.$onInit = function () {
            ...
            $scope.$on('$InsertText_AceEditorAms' + (vm.id || ''), function (_default, input,offset) {
                data.editor.insert(input);
                if(offset){
                    var positon=data.editor.getCursorPosition();
                    data.editor.moveCursorTo(positon.row,positon.column-offset);
                }
                data.editor.focus();
            })
        ...
        $rootScope.global.$watch.push($scope.$watch('$ctrl.watchModel', data.fun.render));

        $rootScope.global.$watch.push($scope.$watch('$ctrl.readOnly', data.fun.readOnly));
