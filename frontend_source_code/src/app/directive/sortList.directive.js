(function(window, angular) {
    ...
    angular.module('eolinker.directive')
         ...
         *该值将用作将这些根连接在一起的标识符。
         *This is where all the logic takes place.
         *If multiple lists should be connected to each other so that elements can move between them, and they have a common ancestor, place this attribute on the element.
         *If you do not, and you still need to be sorted by the behavior, you must provide the value of the attribute.
         *This value will be used as an identifier to link these roots together.
         */
        .directive('svRoot', [function() {
            function shouldBeAfter(elem, pointer, isGrid) { //转换节点时最低位置限制 The lowest position limit when converting nodes
                return isGrid ? elem.x - pointer.x < 0 : elem.y - pointer.y < 0;
            }

            function getSortableElements(key) { //获取排序节点 Get the sort node
                return ROOTS_MAP[key];
            }

            function removeSortableElements(key) { //移除排序节点 Remove the sort node
                delete ROOTS_MAP[key];
            }

            var sortingInProgress;
            var ROOTS_MAP = Object.create(null); //外容器所包含的排序节点集 The outer container contains the sorted node set
            ...
            return {
                ...
                controller: ['$scope', '$attrs', '$interpolate', '$parse', function($scope, $attrs, $interpolate, $parse) {
                    ...
                    var that = this;
                    var candidates; // 设置可能目的地址集 Set the destination address set
                    var $placeholder; // 占位符节点 Placeholder node
                    var options; // 排序选项 Sorting options
                    var $helper; // 协助节点 - 用鼠标指针拖动的节点 Help node - the node that is dragged with the mouse pointer

                    var $original; // 原始节点 The original node
                    var $target; // 最后完美目的地址 The last perfect destination address
                    var isGrid = false;//是否为网格结构 Whether it is a grid structure
                    var onSort = $parse($attrs.svOnSort);//作为该属性的值传递的表达式将在元素排序后顺序更改时进行计算。The expression passed as the value of the attribute is evaluated when the element is sorted later.

                    // ----- 参考 https://github.com/angular/angular.js/issues/8044
                    $attrs.svOnStart = $attrs.$$element[0].attributes['sv-on-start'];//该表达式作为一个当用户开始移动元素时被计算的参数值传递。The expression is passed as a parameter value that is calculated when the user starts moving the element.
                    $attrs.svOnStart = $attrs.svOnStart && $attrs.svOnStart.value;

                    $attrs.svOnStop = $attrs.$$element[0].attributes['sv-on-stop'];//该表达式作为一个当用户结束移动元素时被计算的参数值传递。The expression is passed as a parameter value that is calculated when the user ends the movement of the element.
                    ...
                    if ($attrs.svGrid) { // sv-grid 是否存在 sv-grid exists
                        isGrid = $attrs.svGrid === "true" ? true : $attrs.svGrid === "false" ? false : null;
                        if (isGrid === null)
                            throw 'Invalid value of sv-grid attribute';
                    } else {
                        // 检查是否至少一个列表具有网格状布局 Check if at least one list has a grid-like layout
                        $scope.$watchCollection(function() { //浅层监视（只监视对象中的第一层元素/属性，如果这些元素/属性还有嵌套属性就不在考虑范围之内）Shallow monitoring (only the first layer of elements in the object / attribute, if these elements / attributes are nested attributes are not considered within the scope)
                    ...
                    this.$moveUpdate = function(opts, mouse, svElement, svOriginal, svPlaceholder, originatingPart, originatingIndex) {
                        ...
                        candidates = [];//候选集 Candidate set
                        if (!$placeholder) {
                            if (svPlaceholder) { // 自定义占位符 Custom placeholders
                                $placeholder = svPlaceholder.clone();
                                $placeholder.removeClass('ng-hide');
                            } else { // 默认占位符 Default placeholder
                    ...
                    this.$drop = function(originatingPart, index, options) {//调整顺序 Adjust the order 
                        ...
                        if (options.revert) {
                            ...
                            var duration = +options.revert * distance / 200; // 恒定速度：持续时间取决于距离 Constant speed: duration depends on distance
                            duration = Math.min(duration, +options.revert); // 但是它不再是options.revert But it is no longer options.revert
                        ...
                        function afterRevert() {//布局恢复函数
                            ...
                            // sv-on-stop 反馈函数 Feedback function
                            onStop($scope, {
                                $part: originatingPart.model(originatingPart.scope),
                                $index: index,
                            ...
                            if ($target) {
                                ...
                                // sv-on-sort 反馈函数 Feedback function
         ...
         *This property should be placed on the element of the container as an element of ngRepeat. The value should be the same as the right-hand expression in the ng-repeat attribute.
         */
        .directive('svPart', ['$parse', function($parse) {
            return {
                restrict: 'A',
                require: '^svRoot', //依赖svRoot指令 Depends on the svRoot directive
                controller: ['$scope', function($scope) {
                    $scope.$svCtrl = this;
                    this.getPart = function() { //获取sv-root $scope.part Get sv-root $ scope.part
                        return $scope.part;
                    };
         ...
         *This property should be placed on the same element as the ng-repeat attribute.
         *Its (optional) value should be an expression that evaluates to an options object.
         *With: mousedown touchstart mousemove touchmove mouseup touchend touchcancel operation
         */
        .directive('svElement', ['$parse', function($parse) { 
            return {
                restrict: 'A',
                require: ['^svPart', '^svRoot'], //依赖svPart以及svRoot指令 Depends on svPart and svRoot instructions
                controller: ['$scope', function($scope) {
                    $scope.$svCtrl = this;
                }],
                link: function($scope, $element, $attrs, $controllers) {
                    ...
                    function onMousedown(e) {//mouseDown函数 mouseDown function
                        touchFix(e);
                        ...
                        clone[0].reposition = function(coords) {//克隆元素重定位 Clone element relocation
                            ...
                            if (containmentRect) {
                                if (targetTop < containmentRect.top + body.scrollTop) // 上边界 Upper boundary
                                ...
                                if (targetLeft < containmentRect.left + body.scrollLeft) // 左边界 Left border
                        ...
                        //mouseUp相应函数 mouseUp corresponding function
         ...
         *This property is optional. If necessary, it can be placed on elements within the sortable element. This element will be the handle of the sort operation.
         */
    ...
    function touchFix(e) {//拖动位置匹配 Drag the position to match
    ...
    function getPreviousSibling(element) {//获取上一级元素 Get the upper level element
        element = element[0];
    ...
    function insertElementBefore(element, newElement) {//在被选元素内部的开头插入新节点 Insert a new node at the beginning of the selected element
    ...
    function elementMatchesSelector(element, selector) {//设置节点匹配选择器 Set the node matching selector
    ...
    /*
     * 简单实现jQuery .add方法 Simple implementation of jQuery .add method
     */
